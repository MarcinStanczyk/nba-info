...
const NBA_TSDB_TO_ESPN: Record<string, string> = {
...
const WNBA_TO_ESPN: Record<string, string> = {
...
const NCAA_TO_ESPN: Record<string, string> = {
...
type LeagueType = "nba" | "wnba" | "ncaa";

function getLeagueAndEspnId(teamId: string): { league: LeagueType; espnId: string } | null {
  if (NBA_TSDB_TO_ESPN[teamId]) return { league: "nba", espnId: NBA_TSDB_TO_ESPN[teamId] };
  if (WNBA_TO_ESPN[teamId]) return { league: "wnba", espnId: WNBA_TO_ESPN[teamId] };
  if (NCAA_TO_ESPN[teamId]) return { league: "ncaa", espnId: NCAA_TO_ESPN[teamId] };
  return null;
}

function getEspnUrl(league: LeagueType, espnId: string): string {
  switch (league) {
...
interface EspnAthlete {
  ...
  birthPlace?: { country?: string; state?: string; city?: string };
  college?: { name?: string; shortName?: string };
}

function parseAthletes(athletes: EspnAthlete[]): EspnPlayer[] {
  return athletes
    ...
    .sort((a, b) => {
      const posOrder: Record<string, number> = { PG: 1, SG: 2, G: 2, SF: 3, F: 3, PF: 4, C: 5 };
 ...
 */
export function hasEspnMapping(teamId: string): boolean {

...
import { getAllTeamsForState, leagueConfig } from "@/backend/src/data/all-basketball-teams";
import type { BasketballTeam } from "@/backend/src/data/all-basketball-teams";
...
export function StateTeamsPanel({ stateCode, onClose }: StateTeamsPanelProps) {
  ...
  const otherTeams = getAllTeamsForState(stateCode);
...
function OtherTeamRow({ team }: { team: BasketballTeam }) {
  ...
  return (
    <Link href={`/team/${team.id}`} className="flex items-center gap-3 rounded-lg px-3 py-2.5 transition-colors hover:bg-secondary/40">
